 ...
 *
 * <p>
 * Now extends {@link BaseDAO} so the simple SELECT/INSERT/UPDATE
...
public class WarehouseDAO extends BaseDAO {
    ...
    private static final String SELECT_WH = "SELECT w.warehouse_id, w.warehouse_code, w.warehouse_name, w.address, "
            + "COALESCE("
            + "    (SELECT u.phone FROM users u "
            + "     WHERE u.warehouse_id = w.warehouse_id "
            + "       AND u.role IN ('WAREHOUSE_STAFF', 'MANAGER') "
            + "       AND u.phone IS NOT NULL AND u.phone != '' AND u.active = 1 "
            + "     ORDER BY CASE u.role WHEN 'WAREHOUSE_STAFF' THEN 1 WHEN 'MANAGER' THEN 2 ELSE 3 END, u.user_id ASC "
            + "     LIMIT 1), "
            + "    w.phone"
            + ") AS phone, "
            + "w.capacity, w.active, w.created_at "
            + "FROM warehouses w";

    private static final String SELECT_ZONE = "SELECT zone_id, warehouse_id, zone_code, zone_name, zone_type, description, active, is_default, capacity "
            + "FROM zones";

    // lấy danh sách tất cả kho trong arraylist theo thứ tự id
     ...
     * với mỗi kho lấy ra thì sẽ truy vấn để đính kèm danh sách zone vào trong
     * Warehouse
     */
    ...
    public boolean insert(Warehouse w, List<Zone> zones) {
        ...
        try {
            ...
            conn.setAutoCommit(false); // tắt chế độ tự động commit để nếu lỗi ở bất kì bước nào sẽ rollback đc và ko
                                       // bị sinh kho rác

            // insert ttin kho vào bảng warehouses
            ...
            int warehouseId = -1;
            try (ResultSet generatedKeys = psWH.getGeneratedKeys()) { // lấy id của kho vừa thêm
            ...
            // thêm các zone vào kho tương ứng (chèn hàng loạt các zone)
        ...
        } catch (SQLException e) { // nếu có lỗi xảy ra thì rollback
            ...
            return false;
        } finally { // đóng kết nối sau khi xử lý xong
    ...
    /**
     * Update warehouse details and reconcile zones (insert/update/delete) in a
     * transaction.
     */
    public boolean update(Warehouse w, List<Zone> newZones) {
        ...
        try {
            ...
            // lấy danh sách các zone hiện tại ra để so sánh
            ...
            try (PreparedStatement psZ = conn.prepareStatement(
                ...
                try (ResultSet rs = psZ.executeQuery()) {
                    ...
                    while (rs.next())
                        currentZones.add(mapZone(rs));
                }
            }
            // thêm zone mới vào set để dễ dàng so sánh
            ...
            String sqlDeleteZone = "DELETE FROM zones WHERE zone_id = ?"; // xoá zone
            String sqlDeactivateZone = "UPDATE zones SET active = 0 WHERE zone_id = ?"; // huỷ kích hoạt zone
    ...
    /**
     * Deletes a non-default zone; falls back to deactivation if the delete is
     * blocked.
     */
